...
### ★★★ 2026-08-10 갱신 — 스튜디오 UI 가 또 바뀌었다 (titan_science 실측)

**옛 도구 `card_w24.py` · `endscreen_w24.py` 는 이제 안 먹는다.** 새 도구 = **`yt_card_endscreen.py`**
...
경로: `/video/<VID>/editor` → **'시작하기'** 클릭(안 누르면 편집기가 안 열린다) →
편집기 안의 **'정보 카드 추가' / '최종 화면 추가' 버튼을 직접** 누른다(행의 '수정'·'+' 는 사라졌다)
→ 드롭다운 **동영상 / 재생목록 / 채널 / 링크** → '동영상'

**★함정 4개 (전부 실측으로 뚫었다)**
1. **최종화면은 기본이 '최근 업로드된 동영상'** 이다. 반드시 **'특정 동영상 선택'** 라디오를
   먼저 눌러야 특정 영상이 걸린다. ★이걸 안 해서 **W23 최종화면이 실패**했다
2. '특정 동영상 선택' 대화상자엔 **검색창이 두 개** — 왼쪽 '내 동영상 검색',
   오른쪽 '다른 채널의 동영상 검색'. `.last` 로 잡으면 오른쪽이라 남의 영상이 뜬다
3. 검색은 `fill()` 이 아니라 **키보드 타이핑**(`keyboard.type`)이어야 결과가 나온다
4. **DB 제목과 실제 유튜브 제목이 다를 수 있다.** 검색어는 API 로 실제 제목을 먼저 확인해라
   (titan 때 DB '열심히 운동하는데' ≠ 실제 '운동만 하면 왜 아플까')

검증: 편집기에서 해당 행이 `+` → **`수정`** 으로 바뀌면 들어간 것.

**★AI 고지 — API 로 이미 된다.** 항목 이름도 '변경된 콘텐츠'가 아니라 **'AI 사용'** 으로 바뀌었다.
`videos.insert` 의 `status.containsSyntheticMedia=True` 를 넣으면 UI 에서 '예'로 들어가 있다.
**응답에서만 `None` 으로 돌아올 뿐**이니 UI 로 다시 만질 필요 없다.
(그전에 "API 로 안 된다"고 적어둔 것은 틀렸다 — 읽기만 안 되는 것이다)

### ★ 업로드 전 10개 조항 자동 점검 — `titan_precheck.py`

큰 편은 올리기 전에 반드시 돌린다(titan 10/10 통과).
① 4K 해상도 ② 워터마크 로고 덮개 ③ 자막 5종 소프트 ④ 언어코드 es-419/zh-Hans
⑤ 번역 통째 실패 없음(en 과 크기 비교) ⑥ 나레이션 개수 ⑦ 길이 정합 ⑧ 오디오 정상
⑨ 썸네일 규격 ⑩ 자막 번인 없음

★`ffmpeg -v error` 를 주면 `volumedetect` 결과까지 지워져 오디오를 0.0dB 로 오판한다. `info` 로 둬라.
