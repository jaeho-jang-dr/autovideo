...
> ### ★★★ 확정 (사장님 2026-08-12)
> **"지금 작동 중인 playwright `locator.click()` 이, 여태 60개 이상의 동영상과 클립을
>  Flow 로 만들면서 내가 본 것 중에 가장 잘 작동하는 스크립트다. 앞으로는 특별한 말
>  없는 한 이것을 모든 영상을 만드는 작업에 필수로 쓸 것이다."**
>
> **기본값이다. 다른 경로를 새로 만들지 마라.** 옛 `flow_driver`/`flow_make_clip` 좌표
> 경로(`flow_make_pose.py`·`flow_make_pose_w24.py`)는 **쓰지 않는다.**

**실제로 도는 스택** (2026-08-12 하루에 동작 클립 19 · 배경 14 · 정지 포즈 13 완주)

| 쓸 것 | 무엇 |
|---|---|
| `flow_cdp_pipeline.py` | 바탕. `launch_chrome`(CDP 9222) · `find_btn` · **`click_marked`** · `open_chip` |
| `W1_2/flow_make_motion6.py` | **동작 동영상** — 기준 이미지 업로드 + 동영상 모드 |
| `W1_2/flow_make_bg.py` | **배경** — 업로드 없음. `set_image_mode()` 로 정지도 만든다 |
| `W1_2/flow_make_pose12.py` | **정지 포즈** — 업로드 + 이미지 모드(Nano Banana) |
| `W1_2/flow_credits.py` | 크레딧 잔량 실측 |

**핵심은 `click_marked()`** — JS 로 찾은 요소에 표식 `data-cc-click` 을 달고 **그 표식으로
locator 를 만들어** 누른다. 실패하면 JS `el.click()` → 최후에 좌표.
**로그에 `[locator]` 가 찍히면 정상**, `[CLICK-FALLBACK]` 이 보이면 UI 가 바뀐 것이다.

★**좌표 클릭은 조용히 실패한다** — 팝오버가 겹치면 클릭이 가로채이는데 로그에는 성공으로
찍히고, 칩이 안 눌린 채 생성이 삼켜져 **180초 뒤 "시간 초과"** 로 죽는다. 간헐적이라
"모델 미선택"으로 오진하기 쉽다(`skid_stop` 은 죽고 같은 배치 `back_flip` 은 통과했다).
...
## ★3-B. ★★★ 배경과 캐릭터는 **상호작용**한다 — 이제부터 항상

★사장님 지시(2026-08-11): **"배경 동영상과 캐릭터가 상호작용하는 것이 극적이다.
이제부터는 항상 그렇게 한다. 배경과 캐릭터 동영상·정지 영상들이 상호작용을 보여 주면서
시나리오와 연동한다."**

배경을 '흔들리는 그림'으로 만들지 마라. 배경 안에 **캐릭터가 건드릴 물체**를 심고,
그 물체가 캐릭터 쪽으로 움직이게 한다. 캐릭터는 그 시각에 반응하고, 나레이션은
그 순간에 그 말을 한다. **셋이 한 사건을 공유한다.**

```
배경   : 물체가 캐릭터 쪽으로 움직인다 (1.5~4.0s)
캐릭터 : 4.0s 에 받는다/놀란다/가리킨다
나레이션: 바로 그 순간 그 단어를 말한다
```

- 씬마다 **사건 하나** — 굴러온다 / 넘어진다 / 솟구친다 / 고개를 내민다 / 불이 켜진다
- 배경 프롬프트에 **시각과 함께** 쓴다 (`1.5-4s: one cucumber rolls out…`)
- 동작 시나리오에 **★맞물림** 줄로 배경 시각과 캐릭터 동작을 못 박는다
- ★**배경을 먼저 만들고 프레임으로 실측**한 뒤 캐릭터를 맞춘다.
  배경이 지시대로 안 나오면 **캐릭터를 배경에 맞춘다** — 배경을 다시 만들지 마라

실례: `W1_2/bg_defs.py` · `W1_2/W1_2_motion.md`
