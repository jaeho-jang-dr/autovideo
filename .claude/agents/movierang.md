...
## 2.5D 캐릭터 모션 엔진 (W1-2 확립 · 2026-08-19)

캐릭터를 배경 위에서 **앞뒤·좌우·사선으로 움직이는** 규격이 섰다.
자세한 것은 `W1_2/MOTION25D_SPEC.md` (DB `project_texts` 에도 있다).

* **원근** — 키와 발 y 는 한 쌍. 키만 줄이면 허공을 걷는다.
  무대는 `stage2d.Stage(<배경키>)` 에게 물어본다. 지평선은 **사람 발밑으로 눈으로** 잰다.
* **보폭** — `strides = |1/(ya−HOR) − 1/(yb−HOR)| / β`, 위치는 `y_at` 로 보간해야 등속으로 보인다.
* **사선 이동** — 전용 컷을 만들지 마라. **x·키·발y 를 한 비트에서 함께 옮기면** 된다.
  ★45도 고정 각도 컷은 Flow 가 못 만든다(여러 번 실패). 각도는 참조 이미지가 정한다.
* **컷 fps** — 비트 9번째 값. 좌표와 독립이라 **발놀림만** 빠르게/느리게 할 수 있다.
* **되감기 금지** — 배경이 모자라면 느리게 정방향(BG_SLOW), 씬은 끝 프레임 유지.
  애초에 씬 길이를 나레이션에 맞추는 것이 정답.
* **블록 = 씬 하나** — 배경 사건과 말을 붙이려면 블록을 씬 단위로 쪼갠다.
  검사는 `python W1_2/check_sync.py [--ko]`.
* **돌아서기는 쓰지 않는다**(사장님 확정). 뒷모습이 필요하면 처음부터 뒷모습 컷으로.
