# -*- coding: utf-8 -*-
"""★W24 YouTube Data API 업로드 — 본편 + 썸네일 + 5개국어 제목/설명 + 태그 (일부공개)
...
os.chdir(ROOT)
PKG = "hangeul_birth_vowels/w24pkg"

LANG = sys.argv[1] if len(sys.argv) > 1 else "ko"
assert LANG in ("ko", "en")

VIDEO = f"hangeul_birth_vowels/hangeul_w24r_full_{LANG}.mp4"
THUMB = f"hangeul_birth_vowels/thumb_w24_{LANG}_1280x720.jpg"
TAGS_F = f"{PKG}/w24_tags.txt"
MANIFEST = f"{PKG}/w24_{LANG}_manifest.json"
ALL = ("ko", "en", "ja", "zh", "es")
YTCODE = {"ko": "ko", "en": "en", "ja": "ja", "zh": "zh-Hans", "es": "es-419"}   # 중국어 간체 = zh-Hans
...
def main():
    ...
    vid = resp["id"]
    yt_quota.log("videos.insert", vid, note=f"W24 {LANG}")
    ...
    con.execute("""INSERT INTO youtube_uploads
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))""",
        ("hangeul_w24r_graduation", "lesson", LANG, vid, f"https://www.youtube.com/watch?v={vid}",
