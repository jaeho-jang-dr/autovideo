#!/bin/bash
# W15 다국어 자막(ja/zh/es) — 소스는 영어판 자막(한글+[로마자]+뜻이 이미 들어있음)
# 한글판·영어판 둘 다 같은 ja/zh/es 자막을 쓴다(화면·타이밍 동일, 오디오만 다름).
cd /d/Entertainments/DevEnvironment/autovideo
export PYTHONIOENCODING=utf-8
SRC="hangeul_birth_vowels/hangeul_w16_stickman_np.en.srt"
OUT="scratch/fixed_subs"
LOG="logs/gen_w16_subs.log"
mkdir -p logs; : > "$LOG"
...
# 영어판 → 한글판 복사(같은 내용)
for code in ja zh es; do
  s="$OUT/W16_2_영어판/${YTNAME[$code]}.srt"
  t="$OUT/W16_1_한글판/${YTNAME[$code]}.srt"
