...
# ★겹모음 — 가로모음과 세로모음이 **한 글자 안에서 자리를 나눠** 앉는다.
#   낱자 획순 테이블에 ㅘㅙㅚㅝㅞㅟㅢ 가 통째로 없어, '외' 를 그리면 초성 ㅇ 만 나오고
#   모음이 사라졌다 (사장님 지적 2026-08-17 "야외 글자 파라메트릭 렌더링이 틀렸다").
#   새 획순을 만들지 않고, **이미 가진 낱자**(ㅗ ㅜ ㅡ / ㅏ ㅓ ㅐ ㅔ ㅣ)를 두 자리에 나눠 쓴다.
COMPLEX = {"ㅘ": ("ㅗ", "ㅏ"), "ㅙ": ("ㅗ", "ㅐ"), "ㅚ": ("ㅗ", "ㅣ"),
           "ㅝ": ("ㅜ", "ㅓ"), "ㅞ": ("ㅜ", "ㅔ"), "ㅟ": ("ㅜ", "ㅣ"),
           "ㅢ": ("ㅡ", "ㅣ")}
...
def _syllable_layout(cho, jung, jong):
    ...
    seq = []
    has_j = jong != " " and jong != ""
    if jung in COMPLEX:         # ★겹모음: 초성 왼위 / 가로부분 그 아래 / 세로부분 오른쪽
        hp, vp = COMPLEX[jung]
        if has_j:
            seq.append((cho, (16, 8, 96, 90)))
            seq.append((hp,  (10, 102, 108, 50)))
            seq.append((vp,  (126, 6, 84, 146)))
            seq.append((jong, (30, 158, 150, 58)))
        else:
            seq.append((cho, (16, 14, 100, 98)))
            seq.append((hp,  (10, 116, 112, 56)))
            seq.append((vp,  (130, 12, 84, 188)))
        return seq
