...
VIDEO = os.path.join(CG, "hangeul_w1_stickman_np_en_burned.mp4")   # 한글자막 박힌 KO(선희) — 리뷰용
FB_JSON = os.path.join(CG, "review_w1_en_feedback.json")
FB_MD = os.path.join(CG, "review_w1_en_feedback.md")
PORT = 8912

def load_scenes():
    p = os.path.join(CG, "hangeul_w1_stickman_np.en.srt")
    scenes = []
...
def save_fb(fb):
    ...
    lines = ["# Correction Feedback — Hangeul W1 (EN / Emma narration)", "",
...
<title>Review — Hangeul W1 (EN)</title>
<style>
...
<div class=left>
  <div class=row><h1>🎬 Review — Hangeul W1 (EN) · 한글의 탄생과 단모음 (선희)</h1>
