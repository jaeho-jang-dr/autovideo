#!/bin/bash
# W16 취미·빈도 인준 신규 포즈 생성 (agy 나노바나나, injun_w10_base 참조로 외모 100% 고정)
# ★사장님 원칙: 옷·신발·얼굴·체형·전체 크기 절대 불변 — 오직 자세만 변경. 병렬 4개.
...
OUT='D:\Entertainments\DevEnvironment\autovideo\home_vocab\w16'
SHEET="hangeul_birth_vowels/w16_injun_pose_prompts.txt"
LOG="logs/gen_injun_w16.log"
mkdir -p home_vocab/w16 logs
...
gen_one() {
  local key="$1"; local action="$2"
  local save="${OUT}\\injun_w16_${key}.png"
  agy -p "${PRE}${action}${SUF} 결과 PNG를 '${save}' 로 저장하라." --dangerously-skip-permissions < /dev/null >> "$LOG" 2>&1
}

echo "=== W16 pose gen start $(date) ===" >> "$LOG"
n=0; pids=()
while IFS='|' read -r key action; do
  key=$(echo "$key" | xargs)
  case "$key" in ""|\#*) continue;; esac
  action=$(echo "$action" | xargs)
  if [[ -f "home_vocab/w16/injun_w16_${key}.png" ]]; then echo "[skip] $key (exists)" >> "$LOG"; continue; fi
  n=$((n+1))
  echo "[gen $n] $key" >> "$LOG"
  gen_one "$key" "$action" &
  pids+=($!)
...
echo "=== W16 pose gen done $(date) : $n dispatched ===" >> "$LOG"
ls -1 home_vocab/w16/injun_w16_*.png 2>/dev/null | wc -l >> "$LOG"
