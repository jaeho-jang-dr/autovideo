# -*- coding: utf-8 -*-
"""W18 선희 음성 DB: KO-W18 스크립트(한/영)의 '…' 인용 한글 표현을 전부 선희로 생성해
   web/public/audio/jamo/ 에 저장 → hangeul_audio_assets 등록.
   ★DB script는 build_w18이 norm_quotes 처리 후 저장한 것 → 클립 파일명이 ensure_scene_audio 조회와 일치.
   ★초안(draft) = edge-tts 선희(무료, 크레딧 절약). 최종화 때는 TTS_ENGINE=azure 로 재생성.
사용: python gen_w18_voicedb.py            (기본 edge 선희)
      TTS_ENGINE=azure python gen_w18_voicedb.py   (최종 Azure 선희)
"""
...
TMP = os.path.join(ROOT, "scratch", "w18_voicedb_tmp"); os.makedirs(TMP, exist_ok=True)
KR = re.compile(r"[가-힣]")
...
spans = set()
for sk, se in cur.execute("SELECT script_kr, script_en FROM scenes WHERE episode='KO-W18'"):
