# -*- coding: utf-8 -*-
"""★W18 전체 업로드(사장님 2026-07-21: "이번 17w는 네가 전체를 다 업로드한다").
비디오(videos.insert)+썸네일+5개국어 자막(API)+지역화+태그+재생목록, 쿼터 자동기록.

W15 스크립트 어댑트. 변경점:
  - 스페인어 코드 es → **es-419**(라틴아메리카), 중국어 zh-Hans(본토). [[subtitle-spanish-latam-chinese-mainland]]
  - 티쳐제이/불국사 W18 취미·반말 패키지.
  - 일부공개(unlisted)로 올리고 1시간 후 4K 확인 뒤 공개 전환.
  - AI 고지 containsSyntheticMedia=True, 아동용=아니요.

사용: python yt_upload_w18.py <ko|en> [--no-subs]
...
THUMB = f"hangeul_birth_vowels/thumb_w18_{LANG}_1280x720.jpg"
TAGS_F = f"{PKG}/w18_tags.txt"
ALL = ("ko", "en", "ja", "zh", "es")
...
def main():
    ...
    vid = resp["id"]
    yt_quota.log("videos.insert", vid, note=f"W18 {LANG}")
    ...
    con.execute("""INSERT INTO youtube_uploads
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))""",
        ("hangeul_w18_emotion", "lesson", LANG, vid, f"https://www.youtube.com/watch?v={vid}",
    ...
    import json
    mpath = f"{PKG}/w18_{LANG}_manifest.json"
    try:
