...
if img is not None:
    ...
    # 2. Erase the original right arm
    # Let's test columns 380 to 635.
    # The source is empty chalkboard (columns 200 to 455, same width of 255 pixels)
    patch_width = 635 - 380  # 255 pixels
    patch_arm = img_resized[280:460, 200:200 + patch_width].copy()
    
    test_img = img_resized.copy()
    test_img[280:460, 380:635] = patch_arm
    
    # 3. Draw active arm
    # Let's test shoulder at (640, 376)
    r_shoulder = (640, 376)
    mx, my = 300, 200  # chalk writing position
    r_hand = (mx, my)
    mid_x = (r_shoulder[0] + r_hand[0]) // 2
    mid_y = (r_shoulder[1] + r_hand[1]) // 2
    r_elbow = (int(mid_x - 15), int(mid_y + 30))
    
    arm_color = (17, 17, 17)  # black line
    stroke_w = 4
    ...
    out_path = os.path.join(out_dir, "test_patched_arm.png")
    cv2.imwrite(out_path, test_img)
    print(f"Test patched arm image saved to {out_path}")
else:
