...
# ==========================================
def draw_mountain_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    # 3 mountain peaks
    pts1 = np.array([[20, 160], [70, 70], [110, 130]], np.int32)
    pts2 = np.array([[80, 160], [120, 50], [160, 120]], np.int32)
    pts3 = np.array([[120, 160], [155, 90], [180, 160]], np.int32)
    ...
    print(f"Generated Mountain Doodle -> {out_path}")
    return out_path

def draw_house_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    # Triangular roof
    pts_roof = np.array([[50, 90], [100, 40], [150, 90]], np.int32)
    cv2.polylines(img, [pts_roof], True, color, thick, lineType=cv2.LINE_AA)
    # Body rectangle
    cv2.rectangle(img, (60, 90), (140, 170), color, thick, lineType=cv2.LINE_AA)
    # Door
    cv2.rectangle(img, (88, 125), (112, 170), color, thick, lineType=cv2.LINE_AA)
    # Chimney
    cv2.line(img, (130, 65), (130, 45), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (130, 45), (140, 45), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (140, 45), (140, 75), color, thick, lineType=cv2.LINE_AA)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, img)
    print(f"Generated House Doodle -> {out_path}")
    return out_path

def draw_pair_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    # Sock 1
    cv2.line(img, (60, 60), (60, 120), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (85, 60), (85, 110), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (60, 60), (85, 60), color, thick, lineType=cv2.LINE_AA)
    cv2.ellipse(img, (85, 125), (25, 15), 30, -90, 90, color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (60, 120), (70, 137), color, thick, lineType=cv2.LINE_AA)
    
    # Sock 2
    cv2.line(img, (120, 60), (120, 120), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (145, 60), (145, 110), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (120, 60), (145, 60), color, thick, lineType=cv2.LINE_AA)
    cv2.ellipse(img, (145, 125), (25, 15), 30, -90, 90, color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (120, 120), (130, 137), color, thick, lineType=cv2.LINE_AA)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, img)
    print(f"Generated Pair Doodle -> {out_path}")
    return out_path

def draw_book_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    pts_l = np.array([[100, 160], [70, 145], [30, 155], [30, 65], [70, 55], [100, 70]], np.int32)
    pts_r = np.array([[100, 160], [130, 145], [170, 155], [170, 65], [130, 55], [100, 70]], np.int32)
    cv2.polylines(img, [pts_l], False, color, thick, lineType=cv2.LINE_AA)
    cv2.polylines(img, [pts_r], False, color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (100, 70), (100, 160), color, thick, lineType=cv2.LINE_AA)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, img)
    print(f"Generated Book Doodle -> {out_path}")
    return out_path
...
def main():
    print("Generating evolution graphic assets (ㅅ, ㅈ, ㅉ, ㅊ)...")
    txt_g = generate_text_lineart("ㅅ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅅ_chalk.png"), font_size=180, stroke_width=6)
    txt_d = generate_text_lineart("ㅈ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅈ_chalk.png"), font_size=180, stroke_width=6)
    txt_dd = generate_text_lineart("ㅉ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅉ_chalk.png"), font_size=180, stroke_width=6)
    txt_t = generate_text_lineart("ㅊ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅊ_chalk.png"), font_size=180, stroke_width=6)
    
    txt_nun = generate_text_lineart("산", os.path.join(ROOT, "assets", "graphics", "letters", "word_산_chalk.png"), font_size=140, stroke_width=5)
    txt_dal = generate_text_lineart("집", os.path.join(ROOT, "assets", "graphics", "letters", "word_집_chalk.png"), font_size=140, stroke_width=5)
    txt_ttang = generate_text_lineart("짝", os.path.join(ROOT, "assets", "graphics", "letters", "word_짝_chalk.png"), font_size=140, stroke_width=5)
    txt_tap = generate_text_lineart("책", os.path.join(ROOT, "assets", "graphics", "letters", "word_책_chalk.png"), font_size=140, stroke_width=5)
    
    obj_snow = draw_mountain_doodle(os.path.join(ROOT, "assets", "graphics", "obj_mountain_chalk.png"))
    obj_moon = draw_house_doodle(os.path.join(ROOT, "assets", "graphics", "obj_house_chalk.png"))
    obj_earth = draw_pair_doodle(os.path.join(ROOT, "assets", "graphics", "obj_pair_chalk.png"))
    obj_tower = draw_book_doodle(os.path.join(ROOT, "assets", "graphics", "obj_book_chalk.png"))
    ...
    sc1_nar_path = "scratch/sfx_scene_1_narration_s.mp3"
    sc2_nar_path = "scratch/sfx_scene_2_narration_s.mp3"
    ...
    gTTS(text="시옷에 이렇게 획을 보태면 다른 자음으로 확장할수 있고", lang="ko").save(sc1_nar_path)
    ...
    # ========================================================
    # 🎥 SCENE 1: 'ㅅ, ㅈ, ㅉ, ㅊ' drawn and fly (13.5s)
    # ========================================================
    scene_1_dur = 13.5
    
    def make_scene_1_frame(t):
        prev_frame_path = os.path.join(ROOT, "assets", "characters", "chalkboard_a_final.png")
        ...
        if t_rel >= 0.5:
            ...
            if t_rel < 2.0:
                ...
                if t_rel < 1.25:
                    ...
                    mx = int(300 - 50 * p)
                    my = int(200 + 100 * p)
                    cv2.line(frame, (300, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                else:
                    p = (t_rel - 1.25) / 0.75
                    mx = int(300 + 50 * p)
                    my = int(200 + 100 * p)
                    cv2.line(frame, (300, 200), (250, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (300, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
            ...
            # 3. ㅉ: t_rel = 5.9 ~ 7.4
                ...
                if t_rel < 6.65:
                    ...
                    if p < 0.33:
                        ...
                        mx = int(240 - 30 * p2)
                        my = int(200 + 100 * p2)
                        cv2.line(frame, (210, 200), (270, 200), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (240, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                    else:
                        p2 = (p - 0.66) / 0.34
                        mx = int(240 + 30 * p2)
                        my = int(200 + 100 * p2)
                        cv2.line(frame, (210, 200), (270, 200), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (240, 200), (210, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (240, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                # Second ㅈ (x=310~370)
                else:
                    p = (t_rel - 6.65) / 0.75
                    ...
                    cv2.line(frame, (210, 200), (270, 200), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (240, 200), (210, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (240, 200), (270, 300), marker_color, marker_thick, cv2.LINE_AA)
                    ...
                    if p < 0.33:
                        ...
                        mx = int(340 - 30 * p2)
                        my = int(200 + 100 * p2)
                        cv2.line(frame, (310, 200), (370, 200), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (340, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                    else:
                        p2 = (p - 0.66) / 0.34
                        mx = int(340 + 30 * p2)
                        my = int(200 + 100 * p2)
                        cv2.line(frame, (310, 200), (370, 200), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (340, 200), (310, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (340, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
            ...
            # 4. ㅊ: t_rel = 8.6 ~ 10.1
                ...
                if p < 0.2:
                    ...
                    mx = int(280 + 40 * p2)
                    my = 170
                    cv2.line(frame, (280, 170), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                elif p < 0.45:
                    p2 = (p - 0.2) / 0.25
                    mx = int(250 + 100 * p2)
                    my = 200
                    cv2.line(frame, (280, 170), (320, 170), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (250, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                elif p < 0.72:
                    p2 = (p - 0.45) / 0.27
                    mx = int(300 - 50 * p2)
    ...
    sub_1 = make_subtitle_clip("시옷에 이렇게 획을 보태면 다른 자음으로 확장할수 있고", scene_1_dur - 1.0, 1.0)
    ...
    output_path = os.path.join(ROOT, "assets", "videos", "jay_whiteboard_t_evolution.mp4")
    ...
    # Save last frame
    last_frame_path = os.path.join(ROOT, "assets", "characters", "chalkboard_t_final.png")
    ...
    if os.path.exists(sqlite_path):
        ...
        project = "jay_t_evolution"
        scene_no = 1
        scene_name = "Jay_Whiteboard_S_Evolution_Video"
        base_image = "assets/characters/jay_writing_board_side_opaque.png"
        
        c_desc = (
            "Hangeul Consonants ㅅ, ㅈ, ㅉ, ㅊ drawn on chalkboard, flying out to become words 산, 집, 짝, 책, "
            "which remain next to their corresponding doodles."
        )
        file_path = "assets/videos/jay_whiteboard_t_evolution.mp4"
