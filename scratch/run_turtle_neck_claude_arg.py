...
prompt = """[프로젝트: autovideo, 스택: Python + MoviePy + gTTS]
...
- 실행 중 Antigravity CLI의 승인 팝업이 나타날 경우를 대비하여, 필요 시 `python scratch/auto_approve.py` 매크로 스크립트를 백그라운드로 먼저 구동한 후에 실행을 하도록 하십시오.
- Playwright 브라우저가 사용자의 화면에 정상적으로 표시(headless=False)되어, 사용자가 생성 및 다운로드 흐름을 실시간 모니터링할 수 있도록 구동하십시오.
- 렌더링 과정이 끊기지 않고 완료될 때까지 터미널 세션을 유지(Wait)해서 완전히 끝내십시오.
[참조 파일]: .harness/context/principles.md
"""

# cmd 리스트에 prompt를 포지셔널 인자로 추가
cmd = ["claude", "--dangerously-skip-permissions", "-p", prompt]

print("Starting Claude Code with positional argument...", flush=True)
