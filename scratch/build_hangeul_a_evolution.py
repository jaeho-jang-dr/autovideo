...
# ==========================================
def draw_door_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    cv2.rectangle(img, (60, 30), (140, 180), color, thick, lineType=cv2.LINE_AA)
    cv2.circle(img, (125, 105), 6, color, -1, lineType=cv2.LINE_AA)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, img)
    print(f"Generated Door Doodle -> {out_path}")
    return out_path

def draw_boat_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    pts = np.array([[30, 120], [170, 120], [140, 165], [60, 165]], np.int32)
    cv2.polylines(img, [pts], True, color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (100, 120), (100, 40), color, thick, lineType=cv2.LINE_AA)
    pts_sail = np.array([[100, 45], [145, 85], [100, 85]], np.int32)
    cv2.polylines(img, [pts_sail], True, color, thick - 2, lineType=cv2.LINE_AA)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, img)
    print(f"Generated Boat Doodle -> {out_path}")
    return out_path

def draw_bread_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    # Bread outline
    cv2.line(img, (50, 150), (150, 150), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (50, 150), (50, 100), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (150, 150), (150, 100), color, thick, lineType=cv2.LINE_AA)
    cv2.ellipse(img, (100, 100), (50, 40), 0, 180, 360, color, thick, lineType=cv2.LINE_AA)
    # Diagonal cuts
    cv2.line(img, (75, 95), (85, 125), color, thick - 2, lineType=cv2.LINE_AA)
    cv2.line(img, (100, 90), (110, 120), color, thick - 2, lineType=cv2.LINE_AA)
    cv2.line(img, (125, 95), (135, 125), color, thick - 2, lineType=cv2.LINE_AA)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, img)
    print(f"Generated Bread Doodle -> {out_path}")
    return out_path

def draw_grass_doodle(out_path):
    img = np.zeros((200, 200, 4), dtype=np.uint8)
    color = (245, 245, 245, 255)
    thick = 6
    cv2.line(img, (50, 170), (70, 90), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (70, 90), (90, 170), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (100, 170), (110, 70), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (110, 70), (130, 170), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (140, 170), (150, 100), color, thick, lineType=cv2.LINE_AA)
    cv2.line(img, (150, 100), (160, 170), color, thick, lineType=cv2.LINE_AA)
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    cv2.imwrite(out_path, img)
    print(f"Generated Grass Doodle -> {out_path}")
    return out_path
...
def main():
    print("Generating evolution graphic assets (ㅁ, ㅂ, ㅃ, ㅍ)...")
    txt_g = generate_text_lineart("ㅁ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅁ_chalk.png"), font_size=180, stroke_width=6)
    txt_d = generate_text_lineart("ㅂ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅂ_chalk.png"), font_size=180, stroke_width=6)
    txt_dd = generate_text_lineart("ㅃ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅃ_chalk.png"), font_size=180, stroke_width=6)
    txt_t = generate_text_lineart("ㅍ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅍ_chalk.png"), font_size=180, stroke_width=6)
    
    txt_nun = generate_text_lineart("문", os.path.join(ROOT, "assets", "graphics", "letters", "word_문_chalk.png"), font_size=140, stroke_width=5)
    txt_dal = generate_text_lineart("배", os.path.join(ROOT, "assets", "graphics", "letters", "word_배_chalk.png"), font_size=140, stroke_width=5)
    txt_ttang = generate_text_lineart("빵", os.path.join(ROOT, "assets", "graphics", "letters", "word_빵_chalk.png"), font_size=140, stroke_width=5)
    txt_tap = generate_text_lineart("풀", os.path.join(ROOT, "assets", "graphics", "letters", "word_풀_chalk.png"), font_size=140, stroke_width=5)
    
    obj_snow = draw_door_doodle(os.path.join(ROOT, "assets", "graphics", "obj_door_chalk.png"))
    obj_moon = draw_boat_doodle(os.path.join(ROOT, "assets", "graphics", "obj_boat_chalk.png"))
    obj_earth = draw_bread_doodle(os.path.join(ROOT, "assets", "graphics", "obj_bread_chalk.png"))
    obj_tower = draw_grass_doodle(os.path.join(ROOT, "assets", "graphics", "obj_grass_chalk.png"))
    ...
    sc1_nar_path = "scratch/sfx_scene_1_narration_m.mp3"
    sc2_nar_path = "scratch/sfx_scene_2_narration_m.mp3"
    ...
    gTTS(text="미음에 이렇게 획을 보태면 다른 자음으로 확장할수 있고", lang="ko").save(sc1_nar_path)
    ...
    # ========================================================
    # 🎥 SCENE 1: 'ㅁ, ㅂ, ㅃ, ㅍ' drawn and fly (13.5s)
    # ========================================================
    scene_1_dur = 13.5
    
    def make_scene_1_frame(t):
        prev_frame_path = os.path.join(ROOT, "assets", "characters", "chalkboard_s_final.png")
        ...
        if t_rel >= 0.5:
            ...
            # 2. ㅂ: t_rel = 3.2 ~ 4.7
                ...
                if p < 0.25:
                    ...
                    my = int(200 + 100 * p2)
                    cv2.line(frame, (250, 200), (250, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (330, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                elif p < 0.75:
                    p2 = (p - 0.5) / 0.25
                    mx = int(250 + 80 * p2)
                    my = 250
                    cv2.line(frame, (250, 200), (250, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (330, 200), (330, 300), marker_color, marker_thick, cv2.LINE_AA)
            ...
            # 3. ㅃ: t_rel = 5.9 ~ 7.4
                ...
                if t_rel < 6.65:
                    ...
                    if p < 0.25:
                        ...
                        cv2.line(frame, (270, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                    elif p < 0.75:
                        p2 = (p - 0.5) / 0.25
                        mx = int(210 + 60 * p2)
                        my = 250
                        cv2.line(frame, (210, 200), (210, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (270, 200), (270, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (210, 250), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                    else:
                        p2 = (p - 0.75) / 0.25
                        mx = int(210 + 60 * p2)
                        my = 300
                        cv2.line(frame, (210, 200), (210, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (270, 200), (270, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (210, 250), (270, 250), marker_color, marker_thick, cv2.LINE_AA)
                ...
                # Second ㅂ (x=310~370)
                    ...
                    cv2.line(frame, (270, 200), (270, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (210, 250), (270, 250), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (210, 300), (270, 300), marker_color, marker_thick, cv2.LINE_AA)
                    if p < 0.25:
                        p2 = p / 0.25
                        mx = 310
                        my = int(200 + 100 * p2)
                        ...
                        cv2.line(frame, (370, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                    elif p < 0.75:
                        p2 = (p - 0.5) / 0.25
                        mx = int(310 + 60 * p2)
                        my = 250
                        cv2.line(frame, (310, 200), (310, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (370, 200), (370, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (310, 250), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                    else:
                        p2 = (p - 0.75) / 0.25
                        mx = int(310 + 60 * p2)
                        my = 300
                        cv2.line(frame, (310, 200), (310, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (370, 200), (370, 300), marker_color, marker_thick, cv2.LINE_AA)
                        cv2.line(frame, (310, 250), (370, 250), marker_color, marker_thick, cv2.LINE_AA)
            ...
            # 4. ㅍ: t_rel = 8.6 ~ 10.1
                ...
                if p < 0.25:
                    ...
                    my = int(200 + 100 * p2)
                    cv2.line(frame, (250, 200), (350, 200), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (280, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                elif p < 0.75:
                    p2 = (p - 0.5) / 0.25
                    mx = 320
                    my = int(200 + 100 * p2)
                    cv2.line(frame, (250, 200), (350, 200), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (280, 200), (280, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (320, 200), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
                else:
                    p2 = (p - 0.75) / 0.25
                    mx = int(250 + 100 * p2)
                    my = 300
                    cv2.line(frame, (250, 200), (350, 200), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (280, 200), (280, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (320, 200), (320, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (250, 300), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
    ...
    sub_1 = make_subtitle_clip("미음에 이렇게 획을 보태면 다른 자음으로 확장할수 있고", scene_1_dur - 1.0, 1.0)
    ...
    output_path = os.path.join(ROOT, "assets", "videos", "jay_whiteboard_a_evolution.mp4")
    ...
    # Save last frame
    last_frame_path = os.path.join(ROOT, "assets", "characters", "chalkboard_a_final.png")
    ...
    if os.path.exists(sqlite_path):
        ...
        project = "jay_a_evolution"
        scene_no = 1
        scene_name = "Jay_Whiteboard_M_Evolution_Video"
        base_image = "assets/characters/jay_writing_board_side_opaque.png"
        
        c_desc = (
            "Hangeul Consonants ㅁ, ㅂ, ㅃ, ㅍ drawn on chalkboard, flying out to become words 문, 배, 빵, 풀, "
            "which remain next to their corresponding doodles."
        )
        file_path = "assets/videos/jay_whiteboard_a_evolution.mp4"
