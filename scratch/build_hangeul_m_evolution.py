...
def main():
    print("Generating evolution graphic assets (ㅁ)...")
    txt_g = generate_text_lineart("ㅁ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅁ_chalk.png"), font_size=180, stroke_width=6)
    txt_kk = generate_text_lineart("ㅂ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅂ_chalk.png"), font_size=180, stroke_width=6)
    txt_k = generate_text_lineart("ㅍ", os.path.join(ROOT, "assets", "graphics", "letters", "letter_ㅍ_chalk.png"), font_size=180, stroke_width=6)
    
    txt_gang = generate_text_lineart("문", os.path.join(ROOT, "assets", "graphics", "letters", "word_문_chalk.png"), font_size=140, stroke_width=5)
    txt_kkot = generate_text_lineart("배", os.path.join(ROOT, "assets", "graphics", "letters", "word_배_chalk.png"), font_size=140, stroke_width=5)
    txt_kong = generate_text_lineart("풀", os.path.join(ROOT, "assets", "graphics", "letters", "word_풀_chalk.png"), font_size=140, stroke_width=5)
    
    obj_river = draw_door_doodle(os.path.join(ROOT, "assets", "graphics", "obj_door_chalk.png"))
    obj_flower = draw_boat_doodle(os.path.join(ROOT, "assets", "graphics", "obj_boat_chalk.png"))
    obj_bean = draw_grass_doodle(os.path.join(ROOT, "assets", "graphics", "obj_grass_chalk.png"))
    ...
    # We use a split patch: y=200~270, x=380~560 for the hand, and y=270~460, x=380~635 for the arm.
    ...
    # ========================================================
    # 🎥 SCENE 1: 'ㅁ' drawn three times and flies (11.0s)
    # ========================================================
    scene_1_dur = 11.0
        ...
        if t >= 0.5:
            if t < 2.0:
                ...
                if p < 0.25:
                    ...
                    cv2.line(frame, (250, 200), (250, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (350, 200), (350, 300), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (250, 250), (350, 250), marker_color, marker_thick, cv2.LINE_AA)
                    cv2.line(frame, (250, 300), (mx, my), marker_color, marker_thick, cv2.LINE_AA)
    ...
    sub_1 = make_subtitle_clip("미음에 이렇게 획을 보태면 다른 자음으로 확장할수 있고", scene_1_dur, 0.0)
    ...
    if has_audio:
        s1_audios = [
            ...
            pop_audio.with_start(6.0),
            whoosh_audio.with_start(7.5),
            pop_audio.with_start(8.5),
            chime_audio.with_start(9.3),
    ...
    output_path = os.path.join(ROOT, "assets", "videos", "jay_whiteboard_m_evolution.mp4")
    ...
    if os.path.exists(sqlite_path):
        ...
        project = "jay_m_evolution"
        scene_no = 1
        scene_name = "Jay_Whiteboard_M_Evolution_Video"
        base_image = "assets/characters/jay_writing_board_side_opaque.png"
        
        c_desc = (
            "Hangeul Consonants ㅁ, ㅂ, ㅍ drawn on chalkboard, flying out to become words 문, 배, 풀, "
            "which remain next to their corresponding doodles."
        )
        file_path = "assets/videos/jay_whiteboard_m_evolution.mp4"
