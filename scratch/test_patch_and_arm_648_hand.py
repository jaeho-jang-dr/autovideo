...
if img is not None:
    ...
    # 2. Erase the original right arm and hand completely (y: 200~460, x: 380~635)
    # The source is empty chalkboard (rows 200~460, cols 200~455)
    patch_width = 635 - 380
    patch_arm = img_resized[200:460, 200:200 + patch_width].copy()
    
    test_img = img_resized.copy()
    test_img[200:460, 380:635] = patch_arm
    
    # 3. Draw active arm starting from the neck/shoulder junction at (648, 280)
    r_shoulder = (648, 280)
    mx, my = 300, 200
    r_hand = (mx, my)
    ...
    out_path = os.path.join(out_dir, "test_patched_shoulder_648_280_full_hand.png")
    cv2.imwrite(out_path, test_img)
    print(f"Test patched shoulder 648_280 full hand saved to {out_path}")
else:
