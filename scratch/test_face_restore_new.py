...
if img is not None:
    ...
    # 1. Erase white circle (y: 170~250, x: 420~500)
    patch_circle = img_resized[170:250, 200:280].copy()
    img_resized[170:250, 420:500] = patch_circle
    
    # 2. New patch coordinates:
    # y: 200~280, x: 380~560 for hand/forearm (avoiding head up to y=280)
    # y: 280~460, x: 380~640 for upper arm (erasing shoulder/upper arm from y=280 onwards)
    patch_hand = img_resized[200:280, 200:380].copy()
    patch_arm = img_resized[280:460, 200:460].copy()
    
    test_img = img_resized.copy()
    test_img[200:280, 380:560] = patch_hand
    test_img[280:460, 380:640] = patch_arm
    
    # 4. Draw active arm starting from (648, 280)
    r_shoulder = (648, 280)
    mx, my = 300, 200
    r_hand = (mx, my)
    ...
    out_path = os.path.join(out_dir, "test_face_restored_new.png")
    cv2.imwrite(out_path, test_img)
    print(f"Test face restored saved to {out_path}")
else:
