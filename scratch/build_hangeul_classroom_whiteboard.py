...
def generate_text_lineart(text, output_path, font_size=180, stroke_width=6, color=(17, 17, 17, 255)):
...
def draw_active_arm(canvas, mx, my, is_erasing=False, stroke_w=4):
    ...
    if is_erasing:
        ...
        # Chalk/Marker line (use dark color on whiteboard)
        cv2.line(canvas, r_hand, (r_hand[0] - 15, r_hand[1] - 10), (17, 17, 17), 6, cv2.LINE_AA)
...
def main():
    ...
    if not os.path.exists(base_img_path):
        print(f"[ERR] Whiteboard image not found at: {base_img_path}")
        return 1
        
    # 1. Base Image Prep & Patches (Direct White Fill Coordinates)
    base_img = cv2.imread(base_img_path)
    base_img_resized = cv2.resize(base_img, (1280, 720))
    
    # Fill the whiteboard regions with pure white (255, 255, 255)
    # Whiteboard active inner area is x: 290 ~ 1000, y: 40 ~ 454
    # 1. Left chalkboard region (x: 290 ~ 530, y: 40 ~ 454) - erases "제이의" and front arm
    base_img_resized[40:455, 290:530] = (255, 255, 255)
    # 2. Right chalkboard region behind Jay (x: 740 ~ 1000, y: 40 ~ 454) - erases "각" without clipping hair
    base_img_resized[40:455, 740:1000] = (255, 255, 255)
    # 3. Precision erases to clean remaining marker parts close to Jay's face
    # y: 170 ~ 200 -> erase up to x: 573 (erases floating residue, head profile is at x=577)
    base_img_resized[170:200, 530:573] = (255, 255, 255)
    # y: 200 ~ 230 -> erase up to x: 570 (cheek is at x: 580)
    base_img_resized[200:230, 530:570] = (255, 255, 255)
    # y: 230 ~ 270 -> erase up to x: 580 (neck is at x: 611)
    base_img_resized[230:270, 530:580] = (255, 255, 255)
    # y: 270 ~ 455 -> erase up to x: 570 (shoulder is at x: 599)
    base_img_resized[270:455, 530:570] = (255, 255, 255)
    
    whiteboard_bg = base_img_resized
    ...
    for idx, item in enumerate(words_data):
        word = item["word"]
        narr_text = item["narr"]
        print(f"Processing segment {idx+1}/5: '{word}'...")

        # A. Word lineart PNG generation (Using dark marker color)
        txt_path = os.path.join(ROOT, "scratch", "temp_audio", f"txt_{idx}.png")
        generate_text_lineart(word, txt_path, font_size=180, stroke_width=6, color=(17, 17, 17, 255))
        text_img = cv2.imread(txt_path, cv2.IMREAD_UNCHANGED)
        th, tw, _ = text_img.shape
        
        # Centering the text on the board
        pos_x = 320 - tw // 2
        pos_y = 250 - th // 2 # Centered vertically within board Y: 40 ~ 454 (around 250)
        ...
        # Default parameters are used to eagerly bind loop variables and prevent lazy evaluation closure bugs
        def make_segment_frame(t, text_img=text_img, pos_x=pos_x, pos_y=pos_y, tw=tw, th=th):
            frame = whiteboard_bg.copy()
    ...
    if os.path.exists(sqlite_path):
        ...
        c_desc = (
            ...
            "('강', '물', '꽃', '불', '차') on the board, explains them, "
