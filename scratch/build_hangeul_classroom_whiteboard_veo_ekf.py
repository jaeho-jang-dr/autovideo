...
def main():
    print("=== Start Whiteboard Veo Video Compilation (ekf / 달 그림자) ===")
    ...
    temp_dir = os.path.join(ROOT, "scratch", "temp_audio")
    os.makedirs(temp_dir, exist_ok=True)
    tts_path = os.path.join(temp_dir, "narr_ekf.mp3")
    ...
    # 4 scenes corresponding to "달", "그", "림", "자"
    for n in range(1, 5):
        clip_path = os.path.join(ROOT, "jay_ekf_whiteboard", f"scene_{n}.mp4")
        if not os.path.exists(clip_path):
            print(f"[ERR] Veo clip not found: {clip_path}")
            return 1
            
        print(f"Loading scene {n}/4: {clip_path}...")
        video_clip = VideoFileClip(clip_path)
        clip_dur = video_clip.duration # Normally ~8.0s
        
        # Audio elements for this specific scene segment
        audio_components = []
        
        # Chalk writing SFX mixed at the writing phase (0.5s ~ 3.5s)
    ...
    # 5. Render final video in 4K resolution (3840x2160)
    ...
    if os.path.exists(sqlite_path):
        ...
        c_desc = (
            "Whiteboard classroom video utilizing Google Flow (Veo) with last frame transition. "
            "JAY sequentially writes '달', '그', '림', '자' on a clean blank whiteboard."
        )
