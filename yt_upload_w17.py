# -*- coding: utf-8 -*-
"""★W17 전체 업로드(사장님 2026-07-21: "이번 17w는 네가 전체를 다 업로드한다").
...
THUMB = f"hangeul_birth_vowels/thumb_w17_{LANG}_1280x720.jpg"
TAGS_F = f"{PKG}/w17_tags.txt"
ALL = ("ko", "en", "ja", "zh", "es")
...
def main():
    ...
    vid = resp["id"]
    yt_quota.log("videos.insert", vid, note=f"W17 {LANG}")
    ...
    con.execute("""INSERT INTO youtube_uploads
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))""",
        ("hangeul_w17_banmal", "lesson", LANG, vid, f"https://www.youtube.com/watch?v={vid}",
    ...
    import json
    mpath = f"{PKG}/w17_{LANG}_manifest.json"
    try:
