...
MASTER = os.path.join(CG,"child_growth.mp4")
ADIR = os.path.join(CG, "_ko_dae" if LANG=="ko" else "_en_alice")
SRT = os.path.join(CG, f"child_growth.{LANG}.srt")
MALGUN=r"C:\Windows\Fonts\malgunbd.ttf"; ARIALBD=r"C:\Windows\Fonts\arialbd.ttf"
FONT = MALGUN if LANG=="ko" else ARIALBD
W,H=1080,1920
SCENES=[0,6,7]
TITLE = {"ko":"우리 아이 키\n얼마나 클까?","en":"How Tall Will\nYour Child Grow?"}[LANG]
...
VID_H=int(W*9/16)  # 16:9 클립을 폭 1080에 맞춤 → 높이 607
segs=[]; auds=[]; t=0.0
bg=Image.new("RGB",(W,H),(214,232,214))
for idx,sc in enumerate(SCENES):
    st,ed,txt=cues[sc]; slot=ed-st
    ap=os.path.join(ADIR,f"{sc:03d}.mp3"); ad=dur(ap) if os.path.exists(ap) else slot
    seglen=ad
    # 비디오 클립(본편에서)
    v=VideoFileClip(MASTER).subclipped(st,ed).without_audio().resized(width=W)
    v=v.with_duration(seglen).with_position(("center", 360))
    # 배경
    bgc=ImageClip(np.array(bg)).with_duration(seglen)
    # 제목(상단)
    ti=text_png(TITLE, 78, (255,255,255,255), W-120, sw=6)
    tclip=ImageClip(np.array(ti)).with_duration(seglen).with_position(("center",70))
    # 자막(하단)
    ci=text_png(txt, 58, (255,230,120,255), W-120, sw=5)
    cclip=ImageClip(np.array(ci)).with_duration(seglen).with_position(("center", 360+VID_H+60))
    comp=CompositeVideoClip([bgc,v,tclip,cclip],size=(W,H)).with_duration(seglen).with_start(t)
    segs.append(comp)
...
out=os.path.join(CG,f"growth_short_{LANG}.mp4")
