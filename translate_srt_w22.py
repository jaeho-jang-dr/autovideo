# -*- coding: utf-8 -*-
"""W22 5개국 자막: en.srt 타이밍 + ja/zh/es 번역(gemini). '한글'·[발음]은 그대로, 설명만 번역.
   ko/en은 그대로 복사. → hangeul_birth_vowels/w22pkg/hangeul_w22_jieun_np.{code}.srt
   ★외국어 자막에도 한글과 로마자 발음기호가 반드시 남아야 한다(검증 출력의 '한글보존' 확인).
   (translate_srt_w21.py 복제 — 경로만 W22로)"""
import re, os, subprocess, shutil
os.chdir(r"D:\Entertainments\DevEnvironment\autovideo")
BASE = "hangeul_birth_vowels"
EN = f"{BASE}/hangeul_w22_jieun_np.en.srt"
KO = f"{BASE}/hangeul_w22_jieun_np.ko.srt"
PKG = f"{BASE}/w22pkg"
...
shutil.copy(KO, f"{PKG}/hangeul_w22_jieun_np.ko.srt")
shutil.copy(EN, f"{PKG}/hangeul_w22_jieun_np.en.srt")
for code, name in LANGS.items():
    tr, miss = translate(texts, name)
    with open(f"{PKG}/hangeul_w22_jieun_np.{code}.srt", "w", encoding="utf-8") as f:
        for b, t in zip(blocks, tr):
            f.write(f"{b['idx']}\n{b['time']}\n{t}\n\n")
    kr = sum(1 for t in tr if re.search(r"[가-힣]", t))
    rom = sum(1 for t in tr if re.search(r"\[[a-z\- ]+\]", t))
    print(f"  {code}: {len(tr)}줄 · 한글보존 {kr}줄 · 발음기호 {rom}줄 · 누락 {len(miss)}{miss[:5]}")
print("자막 5종 완료 →", PKG)
