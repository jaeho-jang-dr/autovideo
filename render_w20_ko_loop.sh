...
OUT="hangeul_birth_vowels/hangeul_w20_injun_np_ko.mp4"
LOG="logs/render_w20_ko.log"
: > "$LOG"
for attempt in $(seq 1 12); do
  if [ -f "$OUT" ]; then break; fi
  echo "=== RENDER ATTEMPT $attempt $(date) ===" >> "$LOG"
  SUB_LANGS=ko WALK_STRIDE_SEC=1.08 EDGE_ACTIVE_VOICE=sunhi PYTHONUTF8=1 python compile_np.py KO-W20 hangeul_w20_injun review ko >> "$LOG" 2>&1
  sleep 2
done
if [ -f "$OUT" ]; then
