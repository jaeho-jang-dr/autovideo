...
VIDEO = f"hangeul_birth_vowels/hangeul_w21_madam_np_{LANG}.mp4"
THUMB = f"hangeul_birth_vowels/thumb_w21_{LANG}_1280x720.jpg"
TAGS_F = f"{PKG}/w21_tags.txt"
ALL = ("ko", "en", "ja", "zh", "es")
...
def main():
    ...
    con.execute("""INSERT INTO youtube_uploads
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))""",
        ("hangeul_w21_describe_people", "lesson", LANG, vid, f"https://www.youtube.com/watch?v={vid}",
         titles[LANG], "unlisted", THUMB, VIDEO, "@drjay-ed", 1, "Education"))
    con.commit(); con.close()
    print(f"✅ DB 기록 (youtube_uploads)")
    print(f"\n▶ 자막은 yt_api.py subs 로 API 업로드 예정 (W21)")
    return vid
