...
수집:  hangeul_birth_vowels/review_w7_feedback.json  +  .md  (감독이 읽어 교정)
"""
import os, re, json
...
VIDEO = os.path.join(CG, "hangeul_w7_stickman_np_ko_burned.mp4")   # KO=Dae 기본트랙
FB_JSON = os.path.join(CG, "review_w7_feedback.json")
FB_MD = os.path.join(CG, "review_w7_feedback.md")
PORT = 8907

def load_scenes():
    p = os.path.join(CG, "hangeul_w7_stickman_np.ko.srt")
    scenes = []
...
def save_fb(fb):
    ...
    lines = ["# 교정 피드백 — W7 인사말·자기소개 (마담제이, 한글 선희)", "",
