# -*- coding: utf-8 -*-
"""업로드 전 10가지 체크 (W22). 하나라도 ✗면 업로드 중단.
   ★자막은 API 매니페스트(langs[].srt)로 올린다 → 체크10은 매니페스트가 실제로 5개 srt +
     5개 제목/설명을 가리키는지 본다(yt_api.do_subs/do_meta가 소비하는 형태 그대로).
   ★체크4는 사장님 수동 업로드용 subs_upload 폴더 + 외국어 자막의 한글·발음기호 보존까지 본다.
   (precheck_w18.py 계승)"""
import os, re, json, subprocess
os.chdir(r"D:\Entertainments\DevEnvironment\autovideo")
PKG = "hangeul_birth_vowels/w22pkg"
VIDS = {"ko": "hangeul_birth_vowels/hangeul_w22_jieun_np_ko.mp4",
        "en": "hangeul_birth_vowels/hangeul_w22_jieun_np_en.mp4"}
THUMBS = {"ko": "hangeul_birth_vowels/thumb_w22_ko_1280x720.jpg",
          "en": "hangeul_birth_vowels/thumb_w22_en_1280x720.jpg"}
SUBS_DIR = {"ko": "subs_upload/W22_1_한글판", "en": "subs_upload/W22_2_영어판"}
...
for name in ("일본어.srt", "중국어(중국).srt", "스페인어.srt"):
    p = f"{SUBS_DIR['ko']}/{name}"
    t = open(p, encoding="utf-8").read() if os.path.exists(p) else ""
    han = len(re.findall(r"[가-힣]", t))
    rom = len(re.findall(r"\[[a-z\- ]+\]", t))
    chk(f"4b. {name} 한글·발음기호 보존", han > 100 and rom > 10, f"한글 {han}자 · 발음기호 {rom}개")
...
# 10. 매니페스트 2개 — langs 5행, srt 5개 실존, 제목·설명 5쌍
n = 0
for L in ("ko", "en"):
    p = f"{PKG}/w22_{L}_manifest.json"
    if not os.path.exists(p):
        chk(f"10. {L} 매니페스트", False, "없음"); continue
    mf = json.load(open(p, encoding="utf-8"))
    langs = mf.get("langs", [])
    srt_ok = sum(1 for e in langs if e.get("srt") and os.path.exists(e["srt"]))
    meta_ok = sum(1 for e in langs if e.get("title") and e.get("desc")
                  and os.path.exists(e["title"]) and os.path.exists(e["desc"]))
    base = sum(1 for e in langs if not e.get("add"))       # 본언어 정확히 1행
    good = len(langs) == 5 and srt_ok == 5 and meta_ok == 5 and base == 1
    chk(f"10. {L} 매니페스트(자막5·메타5·본언어1)", good, f"langs={len(langs)} srt={srt_ok} meta={meta_ok} base={base}")
    n += good
