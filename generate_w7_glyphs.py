#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""generate_w7_glyphs.py — 7주차 "인사말과 자기소개" 단어/문구 글리프 PNG + DB 등록.
재실행: python generate_w7_glyphs.py
"""
import os
import sys
import sqlite3
...
def reg(cur, has, name_kr, name_en, typ, fp):
    ...
    if has:
        cur.execute("INSERT INTO assets (name_kr, name_en, type, file_path, flow_prompt, created_at) "
                    "VALUES (?,?,?,?,?, datetime('now'))", (name_kr, name_en, typ, fp, "generate_w7_glyphs.py"))
    else:
        cur.execute("INSERT INTO assets (name_kr, name_en, type, file_path, flow_prompt) VALUES (?,?,?,?,?)",
                    (name_kr, name_en, typ, fp, "generate_w7_glyphs.py"))
...
def main():
    ...
    con.commit()
    con.close()
    print(f"{n} W7 glyphs generated + registered")
