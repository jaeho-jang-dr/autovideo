# -*- coding: utf-8 -*-
"""W24 5개국 자막: en.srt 타이밍 + ja/zh/es 번역(gemini). '한글'·[발음]은 그대로, 설명만 번역.
   ko/en은 그대로 복사. → hangeul_birth_vowels/w24pkg/hangeul_w24r_full.{code}.srt
   ★외국어 자막에도 한글과 로마자 발음기호가 반드시 남아야 한다(검증 출력의 '한글보존' 확인).
   (translate_srt_w22.py 복제 — 경로만 W24로)"""
import re, os, subprocess, shutil
os.chdir(r"D:\Entertainments\DevEnvironment\autovideo")
BASE = "hangeul_birth_vowels"
EN = f"{BASE}/hangeul_w24r_full.en.srt"
KO = f"{BASE}/hangeul_w24r_full.ko.srt"
PKG = f"{BASE}/w24pkg"
...
shutil.copy(KO, f"{PKG}/hangeul_w24r_full.ko.srt")
shutil.copy(EN, f"{PKG}/hangeul_w24r_full.en.srt")
for code, name in LANGS.items():
    tr, miss = translate(texts, name)
    with open(f"{PKG}/hangeul_w24r_full.{code}.srt", "w", encoding="utf-8") as f:
