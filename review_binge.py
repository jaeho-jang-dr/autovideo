...
수집:  binge_watching/review_binge_feedback.json  +  .md  (감독이 읽어 교정)
"""
import os, re, json
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

ROOT = os.path.dirname(os.path.abspath(__file__))
CG = os.path.join(ROOT, "binge_watching")
VIDEO = os.path.join(CG, "binge_watching.mp4")   # KO=Dae 기본트랙
FB_JSON = os.path.join(CG, "review_binge_feedback.json")
FB_MD = os.path.join(CG, "review_binge_feedback.md")
PORT = 8905

def load_scenes():
    p = os.path.join(CG, "binge_watching.ko.srt")
    scenes = []
...
<div class=left>
  <div class=row><h1>🎬 교정 리뷰 — 정주행 동영상 (한글자막)</h1>
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path == "/" or self.path.startswith("/index"):
                ...
                vtt = os.path.join(CG, "binge_watching.ko.vtt")
