# -*- coding: utf-8 -*-
"""한국어 제목/설명 번역 추가: /translations → 한국어행 '제목 및 설명' 셀 클릭 → 입력 → 게시.
...
# 파라미터화: [2]=한국어제목 [3]=한국어설명파일 (없으면 아이키성장 기본)
...
with sync_playwright() as pw:
    ...
    # 한국어 행 Y + '제목 및 설명' 열 X 로 셀 클릭
    try:
        en = pg.get_by_text("한국어", exact=True).first
        eb = en.bounding_box()
        ...
        log(f"한국어 제목·설명 셀 클릭 좌표=({round(hx)},{round(rowy)})")
