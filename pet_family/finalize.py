...
TARGET_DIR = os.path.dirname(os.path.abspath(__file__))   # pet_family/
...
OUTPUT_VIDEO = os.path.join(TARGET_DIR, "pet_family.mp4")
PROFILE = os.path.join(ROOT_DIR, "assets", "profiles", "dynamic_active.json")

TOTAL_SCENES = 20  # Scene 0 ~ 19
...
# We copy them back into pet_family/ for compilation.
...
def get_missing_scenes():
    missing = []
    for i in range(0, TOTAL_SCENES):  # 0 .. 19
...
def main():
    print("=" * 60)
    print("  '우리 홍시는 내 가족입니다' 통합 복구 및 렌더링 파이프라인 (20씬)")
    print("=" * 60)
    ...
    if missing:
        ...
        for scene_num in missing:
            ...
            try:
                ...
                if res.returncode == 0:
                    # autoveo_flow.py 가 prompts_for_veo/scene_X.mp4 로 저장하므로 pet_family/ 로 복사한다.
    ...
    print("\n[✔] 축하합니다! 0~19번 20개 씬이 누락 없이 모두 준비되었습니다.")
