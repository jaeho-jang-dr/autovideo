...
수집:  child_growth_science/review_growth_en_feedback.json  +  .md  (감독이 읽어 교정)
"""
import os, re, json
...
VIDEO = os.path.join(CG, "child_growth_en_review.mp4")  # EN=Alice 기본트랙
FB_JSON = os.path.join(CG, "review_growth_en_feedback.json")
FB_MD = os.path.join(CG, "review_growth_en_feedback.md")
PORT = 8902

def load_scenes():
    p = os.path.join(CG, "child_growth.en.srt")
    scenes = []
...
def save_fb(fb):
    ...
    lines = ["# Correction Feedback — How Tall Will Your Child Grow? (English Alice narration)", "",
...
<title>Correction Review — Child Growth (English)</title>
<style>
...
<div class=left>
  <div class=row><h1>🎬 Review — How Tall Will Your Child Grow? (English / Alice)</h1>
    <span class=jump>영상 정지 → 메모 입력 → 추가</span></div>
  <video id=vid controls preload=metadata crossorigin=anonymous>
    <source src="/video" type="video/mp4">
    <track id=trk kind=subtitles srclang=en label="English" src="/subs" default>
  </video>
...
e.g. 'mispronounced', 'add a pause here'"></textarea>
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path == "/" or self.path.startswith("/index"):
                ...
                vtt = os.path.join(CG, "child_growth.en.vtt")
