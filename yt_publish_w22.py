# -*- coding: utf-8 -*-
"""W22 발행: 4K 업스케일 확인 → 확인되면 일부공개(unlisted)를 공개(public)로 전환.
video_id 는 업로드 때 기록된 매니페스트(w22pkg/w22_{ko,en}_manifest.json)에서 읽는다.

동작:
  - yt-dlp -F 로 각 영상 2160p(4K) 존재 여부 확인(쿼터 0).
  - 4K 있으면 privacyStatus=public (videos.update, status part만).
  - 4K 아직이면 공개 보류(다음 실행 재확인).
사용: python yt_publish_w22.py [--force]
(yt_publish_w19.py 계승)"""
...
def load_ids():
    ids = {}
    for lang in ("ko", "en"):
        p = f"{PKG}/w22_{lang}_manifest.json"
        if os.path.exists(p):
...
def main():
    ids = load_ids()
    if not ids:
        raise SystemExit("매니페스트에 video_id 없음 — 먼저 yt_upload_api_w22.py 로 업로드하세요.")
    y = yt()
