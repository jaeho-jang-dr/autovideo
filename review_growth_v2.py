...
VIDEO = os.path.join(CG, "child_growth_dub_v2.mp4")   # KO=Dae 기본트랙
...
def save_fb(fb):
    ...
    lines = ["# 교정 피드백 — 우리 아이 키 얼마나 자랄까요? (한글 Dae 나레이션)", "",
...
<title>교정 리뷰 — 아이 키 성장 (수정본 v2)</title>
<style>
...
<div class=left>
  <div class=row><h1>🎬 교정 리뷰 — 우리 아이 키 얼마나 클까? (수정본 v2 · 아웃트로 제거)</h1>
    ...
    class H(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path == "/" or self.path.startswith("/index"):
                ...
                vtt = os.path.join(CG, "child_growth_v2.ko.vtt")
