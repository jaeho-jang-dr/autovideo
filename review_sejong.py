#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""review_sejong.py — 세종대왕과 한글 본편 교정 리뷰 도구 (review_growth.py 복제).
왼쪽: 영상 재생(정지 가능)+한글자막(항상 켜짐) / 오른쪽: 씬 타임라인 + 시점별 교정 메모.
영상: 유튜브 게시 4K 마스터(sub_ko.srt와 타이밍 일치), 이 서버가 HTTP Range로 직접 서빙.
실행:  python review_sejong.py
열기:  http://localhost:8903
수집:  sejong_film/main/pkg/review_sejong_feedback.json  +  .md  (감독이 읽어 교정)
"""
import os, re, json
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

ROOT = os.path.dirname(os.path.abspath(__file__))
CG = os.path.join(ROOT, "sejong_film", "main", "pkg")
# 교정본: audio_11(자모=이름 스펠링, 훈민정음 교정)로 재렌더한 KO 마스터 + 재생성 sub_ko.srt(타이밍 일치)
VIDEO = os.path.join(ROOT, "sejong_film", "main", "sejong_master_video.mp4")
SRT = os.path.join(CG, "sub_ko.srt")
VTT = os.path.join(CG, "sub_ko.vtt")
FB_JSON = os.path.join(CG, "review_sejong_feedback.json")
FB_MD = os.path.join(CG, "review_sejong_feedback.md")
PORT = 8903

def srt_to_vtt():
    """sub_ko.srt → sub_ko.vtt (없거나 오래됐으면 생성)."""
    try:
...
def save_fb(fb):
    ...
    lines = ["# 교정 피드백 — 세종대왕과 한글 창제 (한국어 본편)", "",
...
<div class=left>
  <div class=row><h1>🎬 교정 리뷰 — 세종대왕과 한글 창제 (한국어 본편)</h1>
