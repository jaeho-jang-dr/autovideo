# -*- coding: utf-8 -*-
"""★W22 YouTube Data API 업로드 — 본편 + 썸네일 + 5개국어 제목/설명 + 태그 (일부공개)
   자막은 업로드 직후 `python yt_api.py subs <VID> <manifest>` 로 5개국어를 API로 올린다
   (사장님 지시 2026-07-27: 자막도 API로).

사용: python yt_upload_api_w22.py <ko|en>
쿼터: videos.insert 1,600 + thumbnails.set 50 ≈ 1,650/편 (자막은 captions.insert 400 × 5)
   ★video_id를 매니페스트에 기록한다 — yt_publish_w22.py(공개 전환)가 이 값을 읽는다.
   (yt_upload_api_w21.py 계승)"""
...
VIDEO = f"hangeul_birth_vowels/hangeul_w22_jieun_np_{LANG}.mp4"
THUMB = f"hangeul_birth_vowels/thumb_w22_{LANG}_1280x720.jpg"
TAGS_F = f"{PKG}/w22_tags.txt"
MANIFEST = f"{PKG}/w22_{LANG}_manifest.json"
ALL = ("ko", "en", "ja", "zh", "es")
...
def main():
    ...
    vid = resp["id"]
    yt_quota.log("videos.insert", vid, note=f"W22 {LANG}")
    ...
    con.execute("""INSERT INTO youtube_uploads
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,datetime('now'))""",
        ("hangeul_w22_travel", "lesson", LANG, vid, f"https://www.youtube.com/watch?v={vid}",
