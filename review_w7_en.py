...
수집:  hangeul_birth_vowels/review_w7_en_feedback.json  +  .md  (감독이 읽어 교정)
"""
import os, re, json
...
VIDEO = os.path.join(CG, "hangeul_w7_stickman_np_en_burned.mp4")   # KO=Dae 기본트랙
FB_JSON = os.path.join(CG, "review_w7_en_feedback.json")
FB_MD = os.path.join(CG, "review_w7_en_feedback.md")
PORT = 8908

def load_scenes():
    p = os.path.join(CG, "hangeul_w7_stickman_np.en.srt")
    scenes = []
...
def save_fb(fb):
    ...
    lines = ["# 교정 피드백 — W7 Greetings (Madam Jay, English)", "",
