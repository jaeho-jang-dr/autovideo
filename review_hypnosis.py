#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""review_hypnosis.py — 최면의 과학 본편 교정 리뷰 도구 (review_growth.py 복제).
왼쪽: 영상 재생(정지 가능)+한글자막(항상 켜짐) / 오른쪽: 씬 타임라인 + 시점별 교정 메모.
영상: 유튜브 게시 4K 마스터(hypnosis_science.ko.srt와 타이밍 일치), 이 서버가 HTTP Range로 직접 서빙.
실행:  python review_hypnosis.py
열기:  http://localhost:8903
수집:  sejong_film/main/pkg/review_hypnosis_feedback.json  +  .md  (감독이 읽어 교정)
"""
import os, re, json
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer

ROOT = os.path.dirname(os.path.abspath(__file__))
CG = os.path.join(ROOT, "hypnosis_science")
# 최면 재작업본(인트로/아웃트로 제거 + Azure 선희/Emma) + 재생성 ko.srt(타이밍 일치)
VIDEO = os.path.join(CG, "hypnosis_brain_science_truth.mp4")
SRT = os.path.join(CG, "hypnosis_science.ko.srt")
VTT = os.path.join(CG, "hypnosis_science.ko.vtt")
FB_JSON = os.path.join(CG, "review_hypnosis_feedback.json")
FB_MD = os.path.join(CG, "review_hypnosis_feedback.md")
PORT = 8904

def srt_to_vtt():
    """hypnosis_science.ko.srt → hypnosis_science.ko.vtt (없거나 오래됐으면 생성)."""
    try:
        if os.path.exists(VTT) and os.path.getmtime(VTT) >= os.path.getmtime(SRT):
            return
    except Exception:
        pass
    txt = open(SRT, encoding="utf-8").read().replace("\r\n", "\n")
    txt = re.sub(r"(\d\d:\d\d:\d\d),(\d\d\d)", r"\1.\2", txt)  # 콤마→점
...
def load_scenes():
    ...
    for i, blk in enumerate(txt.strip().split("\n\n")):
        ...
        scenes.append({"seq": i, "start": round(st, 2), "end": round(ed, 2), "cap": " ".join(L[2:])[:60]})
    return scenes
...
def save_fb(fb):
    ...
    lines = ["# 교정 피드백 — 최면의 과학 창제 (한국어 본편)", "",
             "> review_hypnosis.py 수집. 감독(Claude)이 이 파일을 읽어 해당 씬만 교정한다.", ""]
...
h1{font-size:15px;margin:0 0 8px;color:#7fb0e0;font-weight:700}
...
.chip:hover{background:#30343c} .chip.cur{background:#7fb0e0;color:#123;font-weight:700}
...
.btn{margin-top:8px;background:#2b5f9f;color:#fff;border:0;border-radius:8px;padding:10px;font-weight:700;cursor:pointer;font-size:14px}
.btn:hover{background:#3574bb}
...
.item .ts{color:#7fb0e0;font-weight:700;cursor:pointer}
...
<div class=left>
  <div class=row><h1>🎬 교정 리뷰 — 최면의 과학 창제 (한국어 본편)</h1>
...
<div class=right><div class=panel>
  ...
  <textarea id=note placeholder="이 시점 나레이션/자막/영상에서 고칠 점을 적어주세요. (정지 후 입력 → 추가)
예: '연도 발음 이상함', '이 장면 자막 오타', '이 컷 너무 빠름'"></textarea>
