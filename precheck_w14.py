# -*- coding: utf-8 -*-
"""업로드 전 10가지 체크 (W14). 하나라도 ✗면 업로드 중단.
   ★이번엔 자막을 사장님이 유튜브 UI로 직접 올리시므로,
     체크4는 '유튜브에 올릴 자막이 subs_upload/ 에 5개씩 준비됐는지'를 본다(API 업로드 안 함)."""
import os, re, subprocess
os.chdir(r"D:\Entertainments\DevEnvironment\autovideo")
PKG = "hangeul_birth_vowels/w14pkg"
# ★compile_np 4K 모드는 접미사 없이 같은 파일명에 3840x2160으로 저장한다
VIDS = {"ko": "hangeul_birth_vowels/hangeul_w14_madam_np_ko.mp4",
        "en": "hangeul_birth_vowels/hangeul_w14_madam_np_en.mp4"}
THUMBS = {"ko": "hangeul_birth_vowels/thumb_w14_ko_1280x720.jpg",
          "en": "hangeul_birth_vowels/thumb_w14_en_1280x720.jpg"}
SUBS_DIR = {"ko": "subs_upload/W14_1_한글판", "en": "subs_upload/W14_2_영어판"}
...
# 10. 매니페스트 2개 + 자막항목 비어있는지(사장님 직접 업로드)
import json
n = 0
for L in ("ko", "en"):
    p = f"{PKG}/w14_{L}_manifest.json"
    if not os.path.exists(p):
        continue
    mf = json.load(open(p, encoding="utf-8"))
    if not mf.get("subs") and len(mf.get("localizations", {})) == 5:
        n += 1
chk("10. 매니페스트(자막 비움·5개국어 메타)", n == 2, f"{n}/2")
