...
### 2) 작업 분담 비율 및 검수 규칙 (2026-06-19 제작자 확정)
*   **기본 비율 70/30**: 제작자 지시가 떨어지면 실제 실행 작업의 **약 70%는 조감독(Gemini)이 CLI로 직접 수행**하고, **약 30%만 감독(Claude)이 직접** 처리하여 작업량 균형을 맞춘다.
*   **위임 → 검수 → 폴백**: 감독은 위임한 작업의 결과를 **반드시 검수**하고, 조감독이 **못 하거나 미흡하면 그때 감독(Claude)이 직접 대체**하여 마무리한다.
*   trivial하거나 즉시 처리가 더 빠른 작업은 예외로 둘 수 있으나, 기본 원칙은 **위임-검수-폴백**을 따른다.
